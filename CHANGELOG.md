...
- **Design Language (DTL)**: a shared typography/glyph vocabulary for every card, receipt and progress renderer lands in `src/telegram/ui.ts` (spec: `docs/DESIGN.md`), grounded in the official Bot API formatting contract. Cards move to a two-line header (icon + bold title + quiet meta strip), bold primary keys with two-space indented secondary rows (relative time + monospace short id), and one semantic per glyph (✅/❌/⏳/⚙️/▶️/▸/•). New capabilities adopted: `<tg-time format="r">` date-time entities render client-localized relative time for `last prompt` lines, and `<blockquote expandable>` folds trajectory overflow steps behind a tap instead of a bare counter. Surfaces migrated: Trajectory (+ `/history` now sends HTML with a plain-text degrade), Todos card (`[status]` tag noise dropped — state lives in icons and completed items are struck through), goal progress card, turn receipt, the Sessions/Projects/Detail/Search cards, and the full second wave — Goals, Queue, Models/Providers/Thinking, Presets, Workspaces/Project picker, and all fourteen misc cards (Plugins, Skills, Subagents, Jobs, Dynamic plugins, Capabilities, Feedback, Mode, Allowed chats, Watch, Telegram settings, About). Two latent HTML-injection 400s fixed on the way: goal objectives and todo content were interpolated raw under `parse_mode: HTML`; both are escaped now.
